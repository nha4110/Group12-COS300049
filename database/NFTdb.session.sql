...
-- Create collections table (uses base_cid, no png_hash/json_hash)
CREATE TABLE collections (
    ...
    base_cid VARCHAR(255) NOT NULL, -- Single IPFS base CID for metadata and images
...
CREATE TABLE assets (
    ...
    price_eth DECIMAL(18,6) NOT NULL, -- Increased precision for ETH values
    contract_address VARCHAR(255) NOT NULL CHECK (contract_address ~* '^0x[a-fA-F0-9]{40}$'),
    image_url VARCHAR(255),
    category VARCHAR(255) NOT NULL,
    tx_hash VARCHAR(66) NOT NULL UNIQUE CHECK (tx_hash ~* '^0x[a-fA-F0-9]{64}$'), -- 66 chars for tx hash (0x + 64 hex)
...
CREATE TABLE transactions (
    ...
    amount_eth DECIMAL(18,6) NOT NULL, -- Increased precision for ETH values
    transaction_type VARCHAR(50) NOT NULL,
    contract_address VARCHAR(255) NOT NULL CHECK (contract_address ~* '^0x[a-fA-F0-9]{40}$'),
    tx_hash VARCHAR(66) NOT NULL UNIQUE CHECK (tx_hash ~* '^0x[a-fA-F0-9]{64}$'), -- 66 chars for tx hash
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (account_id) REFERENCES users(account_id) ON DELETE SET NULL
);

-- Insert the "out" collection
