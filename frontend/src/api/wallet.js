...
export async function getWalletBalance(walletAddress) {
  ...
  try {
    ...
    if (response.data.success) {
      return { success: true, balance: parseFloat(response.data.balance).toFixed(4) };
    }
    throw new Error("Backend fetch failed");
  } catch (backendError) {
    console.warn("Backend balance fetch failed, falling back to Web3:", backendError.message);
    try {
      ...
      console.error("❌ Error fetching wallet balance:", web3Error);
